...
# deadline is spent. 30 (not 50): imports + account fetch + reconciliation +
# regime all run BEFORE the deadline starts, so on a slow GitHub runner that
# overhead alone is ~10-20s -- 50s budget + 20s overhead = 70s > the 60s
# shell cap (`timeout 60` in run-bot.yml) = exit 124. With 30s the worst
# case is ~30s + 20s = ~50s, always inside the cap. Normal runs (8-34s) are
# unaffected; the budget only binds when something hangs.
RUN_BUDGET_SECONDS = float(os.environ.get("RUN_BUDGET_SECONDS", 30))
