...
The Gemini "attempt" timestamp is marked BEFORE the call is made, not
after a successful response -- this ensures a failed/quota-exceeded call
still respects the full cooldown before retrying, instead of retrying
every 2 minutes during an outage.
"""
...
def run():
    ...
    if _should_call_gemini():
        log_lines.append("Gemini call interval reached -- generating new trade ideas.")
        _mark_gemini_called()
